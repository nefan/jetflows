...
"""
Generate simple images of curves and lines for testing 2nd order features
"""
...
fixed = '../data/sincecim1.npy'
moving = '../data/sinceim2.npy'
pointsPerAxis = 1
border = 100
order = 2
weights = [0*1.5*1e-3, 1]
weights = weights/np.sum(weights)
smoothing = 0.05 # this looks good!
splineS = 2**4
SIGMAF = 2.**(-0)
h = 1./(pointsPerAxis+1)
...
#plt.show(block=True)
plt.gcf().savefig('../data/sinecim1.png')
np.save(fixed,im1)

plt.imshow(im2, interpolation='nearest')
#plt.show(block=True)
plt.gcf().savefig('../data/sinecim2.png')
np.save(moving,im2)
