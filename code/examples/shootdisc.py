...
"""
Deforming at disc to a different size disc
...
fixed = '../data/shootdisc1.npy'
moving = '../data/shootdisc2.npy'
pointsPerAxis = 1
border = 110
order = 2
weights = [0*1e-5, 1]
...
#plt.show(block=True)
plt.gcf().savefig('../data/shootdisc1.png')
np.save(fixed,im1)
plt.imshow(im2, interpolation='nearest')
#plt.show(block=True)
plt.gcf().savefig('../data/shootdisc2.png')
np.save(moving,im2)
...
# initial conditions
#p = np.array([[1.0,0.0]])
mu_1 = 30.*np.array([[[1.0 , 0.0],[0.0,1.0]]])
